...
// keep reference to current realtime subscription for tracking
...
function renderMenuItems(items) {
  ...
  const regularPrice = Number(item.price_regular ?? item.price);
    ...
    sizeOptions[0].querySelector('.size-price').textContent =
      `৳${regularPrice.toFixed(2)}`;
    sizeOptions[1].querySelector('.size-price').textContent =
...
function filterAndRender() {
  ...
  if (currentFilter.category !== 'All') {
    filteredItems = filteredItems.filter(
      (item) => item.category === currentFilter.category
    );
  }
  ...
  if (currentFilter.dietary !== 'All Dietary Options') {
    filteredItems = filteredItems.filter((item) =>
      item.dietary_tags
        ?.toLowerCase()
        .includes(currentFilter.dietary.toLowerCase())
    );
  }
...
function renderCartItems() {
  ...
  if (cart.length === 0) {
    cartBody.innerHTML =
      '<div class="cart-empty"><i data-lucide="shopping-bag"></i><p>Your cart is empty</p></div>';
    lucide.createIcons();
    return;
  }
  ...
  cart.forEach((item) => {
    ...
    cartItemEl.querySelector('.cart-item-size').textContent =
      `Size: ${item.size.charAt(0).toUpperCase() + item.size.slice(1)}`;
    cartItemEl.querySelector('.cart-item-price-each').textContent =
      `৳${item.price.toFixed(2)} each`;
    cartItemEl.querySelector('.quantity-text').textContent = item.quantity;
    cartItemEl.querySelector('.cart-item-subtotal').textContent =
  ...
  cartFooter.innerHTML = `
    <div class="checkout-form">
      ...
      <div class="form-group">
        ...
        <textarea id="order-notes" placeholder="Any special requests?" rows="3"></textarea>
      </div>
      <div class="cart-total-section">
        <span>Total:</span>
        <span>৳${totalPrice.toFixed(2)}</span>
      ...
      <div class="form-actions">
        <button id="back-to-cart" class="checkout-btn back-btn">Back</button>
        <button id="place-order" class="checkout-btn">Place Order</button>
      </div>
    </div>`;
}

/* ---------------------- PLACE ORDER + REALTIME TRACKING ---------------------- */
...
async function placeOrder(orderDetails) {
  ...
  const itemsToInsert = orderDetails.items.map((cartItem) => ({
    ...
    item_name: `${cartItem.name} (${cartItem.size
      .charAt(0)
  ...
  const trackingOrder = {
    id: orderData.id, // REAL DB ORDER ID
...
// subscribe to realtime changes for this order
function showOrderTracking(order) {
  ...
  const channel = supabase
    ...
    .on(
      ...
      (payload) => {
        console.log('Payment inserted for order', payload.new);
        // show "Preparing" (Received + Preparing become active)
        renderOrderTracking(order, 'preparing');
      }
    )
    // when staff updates the order status
    .on(
      'postgres_changes',
      {
        event: 'UPDATE',
      ...
      (payload) => {
        ...
        // you can adjust these mappings to whatever statuses you use in staff dashboard
        if (status === 'Delivery Complete' || status === 'Completed') {
          renderOrderTracking(order, 'complete');
        } else if (status === 'Preparing') {
          renderOrderTracking(order, 'preparing');
        } else if (status === 'Ready') {
          renderOrderTracking(order, 'ready');
        }
      }
    )
    .subscribe((status) => {
...
// status can be: 'received' | 'preparing' | 'ready' | 'complete'
  ...
  steps.forEach((step, index) => {
    ...
    stepsHtml += `
      <div class="status-step">
        ...
        <p class="status-label ${isActive ? 'active' : 'inactive'}">
          ${step.label}
        </p>
        ${isActive ? '<i data-lucide="check-circle-2" class="status-check"></i>' : ''}
      </div>`;
  });

  orderTrackingContent.innerHTML = `
    <div class="modal-header">
      <h2 class="modal-title">Order Tracking</h2>
      <button id="close-tracking-button" class="modal-close-button">
        <i data-lucide="x"></i>
      </button>
    </div>
    <div class="modal-body">
      <div class="order-tracking-info">
        <p>Order ID: <span class="order-id">${order.id
          .slice(0, 8)
          .toUpperCase()}</span></p>
      ...
      ${
        status === 'complete'
          ? '<div class="order-complete-message">Thank you for your order!</div>'
          : ''
      }
    </div>`;

  const closeBtn = document.getElementById('close-tracking-button');
  if (closeBtn) {
...
function setupEventListeners() {
  ...
  menuGrid.addEventListener('click', (e) => {
    ...
    if (e.target.closest('.info-btn')) {
      card
        .querySelector('.card-nutrition-details')
  ...
  cartBody.addEventListener('click', (e) => {
    ...
    if (!item && !['proceed-to-checkout', 'back-to-cart', 'place-order'].includes(e.target.id)) {
      return;
    }

    if (e.target.closest('.plus-btn')) {
      updateCartQuantity(itemId, itemSize, item.quantity + 1);
    }
    if (e.target.closest('.minus-btn')) {
      updateCartQuantity(itemId, itemSize, item.quantity - 1);
    }
    if (e.target.closest('.remove-btn')) {
      updateCartQuantity(itemId, itemSize, 0);
    }
